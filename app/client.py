...
def dashboard():
    ...
    failed_payments = (
        ...
        .filter_by(customer_email=customer.email, status="failed")
        .count()
    )
    pending_payments = (
        db.session.query(Payment)
        .filter_by(customer_email=customer.email, status="pending")
        .count()
    )
...
def billing_receipt(invoice_id):
    ...
    return render_template(
        "client_receipt.html", invoice=invoice, subscription=subscription
    )
