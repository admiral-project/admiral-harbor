...
from app.paypal import PayPalError, create_subscription, get_subscription, verify_webhook_signature
...
def _local_catalog():
    return db.session.query(CatalogApp).filter_by(catalog_enabled=True).order_by(CatalogApp.sort_order.asc(), CatalogApp.name.asc()).all()
...
def _customer_subscriptions(customer):
    return db.session.query(Subscription).filter_by(customer_email=customer.email).order_by(Subscription.created_at.desc()).all()


def _customer_instances(customer):
    return db.session.query(CustomerApp).filter_by(customer_email=customer.email).order_by(CustomerApp.created_at.desc()).all()
...
def _course_price(course_id, tier_name):
    ...
    discount = db.session.query(AppCourseTierDiscount).filter_by(app_course_id=course.id, tier_name=tier_name).one_or_none()
...
def _provision_from_order(order):
    ...
    subscription = Subscription(
        ...
        next_billing_at=order.next_billing_at or (datetime.utcnow() + timedelta(days=30)).date().isoformat(),
...
def index():
    ...
    return render_template("index.html", apps=apps, overdue_policy=overdue_policy(current_app.config), customer=current_customer())
...
def app_detail(slug):
    ...
    courses = db.session.query(AppCourse).filter_by(app_slug=slug, active=True).order_by(AppCourse.course_code.asc()).all()
    return render_template("app_detail.html", app=app.as_dict(), remote=remote, lms_enabled=settings.enabled, courses=courses, customer=current_customer())
...
def deploy_app(slug):
    ...
    if not requires_billing:
        existing_free = db.session.query(CustomerApp).filter_by(
            customer_email=customer.email,
            app_slug=slug,
        ).filter(
    ...
    local_tier = db.session.query(CatalogAppTier).join(CatalogApp).filter(
    ...
    db.session.commit()
    _audit(customer.email, "order_created", "order", order.order_id,
           f"Order for {slug} ({tier_name}) — ${total_cents/100:.2f}")

    if requires_billing:
        try:
            plan_id = paypal_plan_id or f"{slug}:{tier_name}"
            return_url = url_for("main.billing_return", order_id=order.order_id, _external=True)
            cancel_url = url_for("main.billing_cancel", order_id=order.order_id, _external=True)
            ...
            db.session.commit()
            approval_link = next((link["href"] for link in paypal_sub.get("links", []) if link["rel"] == "approve"), None)
            if approval_link:
...
def billing():
    ...
    invoices = db.session.query(Invoice).filter_by(customer_email=customer.email).order_by(Invoice.created_at.desc()).all()
    return render_template("billing.html", orders=orders, subscriptions=subscriptions, invoices=invoices, customer=customer)
...
def billing_return():
    ...
    if order.status == "paid":
        return redirect(url_for("main.billing_receipt", invoice_id=order.subscription_external_id or ""))
...
def billing_receipt(invoice_id):
    ...
    return render_template("receipt.html", invoice=invoice, subscription=subscription, customer=customer)
...
def paypal_webhook():
    ...
    subscription = db.session.query(Subscription).filter_by(paypal_subscription_id=subscription_id).one_or_none()
    if subscription is None:
    ...
    if event_type == "PAYMENT.SALE.COMPLETED":
        tax_cents = int(subscription.monthly_price_cents * subscription.tax_percent / 100)
        ...
        subscription.next_billing_at = (datetime.utcnow() + timedelta(days=30)).date().isoformat()
    ...
    db.session.commit()
    if event_type == "PAYMENT.SALE.COMPLETED":
        _audit(subscription.customer_email, "billing_payment_received", "subscription", subscription.external_id,
               f"Payment received for {subscription.app_slug} ({subscription.tier_name})")
    elif event_type in ("BILLING.SUBSCRIPTION.ACTIVATED",):
        _audit(subscription.customer_email, "subscription_activated", "subscription", subscription.external_id,
               f"Subscription activated for {subscription.app_slug}")
    elif event_type in ("BILLING.SUBSCRIPTION.CANCELLED", "BILLING.SUBSCRIPTION.EXPIRED"):
        _audit(subscription.customer_email, "subscription_cancelled", "subscription", subscription.external_id,
...
def instance_detail(instance_id):
    ...
    try:
        ...
        backups = []
    uploaded = db.session.query(UploadedBackup).filter_by(customer_email=customer.email, app_slug=instance.app_slug).order_by(UploadedBackup.created_at.desc()).all()
    events = db.session.query(InstanceEvent).filter_by(instance_id=instance.instance_id, customer_email=customer.email).order_by(InstanceEvent.created_at.desc()).all()
    incidents = db.session.query(SupportIncident).filter_by(instance_id=instance.instance_id, customer_email=customer.email).order_by(SupportIncident.created_at.desc()).all()
    remote_state = None
    try:
...
def instance_action(instance_id):
    ...
    try:
        ...
        db.session.commit()
        _audit(customer.email, f"instance_{requested}", "instance", instance_id,
...
def upload_backup(instance_id):
    ...
    size = 0
    try:
        with tempfile.NamedTemporaryFile(delete=False, dir=destination, prefix=f".{filename}.", suffix=".upload") as temp_file:
    ...
    db.session.commit()
    _audit(customer.email, "backup_uploaded", "backup", backup.backup_id,
...
def download_uploaded_backup(backup_id):
    customer = current_customer()
    backup = db.session.query(UploadedBackup).filter_by(backup_id=backup_id, customer_email=customer.email).one_or_none()
    if backup is None:
...
def request_restore(instance_id):
    ...
    if source_kind == "uploaded":
        uploaded = db.session.query(UploadedBackup).filter_by(backup_id=source_backup_id, customer_email=customer.email).one_or_none()
        if uploaded is None:
    ...
    db.session.commit()
    _audit(customer.email, "restore_requested", "instance", instance_id,
...
def enroll_course(course_id):
    ...
    subscription = db.session.query(Subscription).filter_by(customer_email=customer.email, app_slug=course.app_slug).order_by(Subscription.created_at.desc()).first()
