...
from app.models import (
    Customer, Subscription, SupportIncident, Payment, AuditLog,
    SubscriptionChange, CustomerReply
)
...
def dashboard():
    ...
    # Get open tickets
    open_tickets = db.session.query(SupportIncident).filter_by(
        customer_id=customer.id
    ).filter(SupportIncident.status != "closed").count()
    
    # Get failed payments
    failed_payments = db.session.query(Payment).filter_by(
        customer_id=customer.id,
        status="failed"
    ).count()
    
    # Get pending payments
    pending_payments = db.session.query(Payment).filter_by(
        customer_id=customer.id,
        status="pending"
    ).count()
    ...
    total_charged_cents = db.session.query(func.sum(Payment.amount_cents)).filter(
...
def subscription_upgrade(subscription_id):
    ...
    if new_tier == subscription.tier:
        ...
        return redirect(url_for("customer.subscription_upgrade_page", subscription_id=subscription_id))
    ...
    flash(f"Subscription upgraded to {new_tier}. Changes effective next billing cycle.", "success")
    return redirect(url_for("customer.subscription_detail", subscription_id=subscription_id))
...
def subscription_cancel_page(subscription_id):
    ...
    return render_template(
        "customer_subscription_cancel.html",
        subscription=subscription
    )
...
def subscription_cancel(subscription_id):
    ...
    if not confirm:
        ...
        return redirect(url_for("customer.subscription_cancel_page", subscription_id=subscription_id))
...
def support_detail_updated(ticket_id):
    ...
    replies = db.session.query(CustomerReply).filter_by(
        ticket_id=ticket.id,
        customer_id=customer.id
    ).order_by(CustomerReply.created_at.asc()).all()
