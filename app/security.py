...
def _warn_default(name, value):
    if not value or value.startswith("dev-") or value == "dev-secret-change-me" or value == "dev-token" or value == "dev-encryption-key":
...
def validate_production_config(config):
    ...
    if not secret_key or secret_key.startswith("dev-") or secret_key == "dev-secret-change-me":
        errors.append("SECRET_KEY must be replaced before production")
    if len(secret_key) < 32:
        errors.append("SECRET_KEY must be at least 32 characters in production")

    if not admiral_token or admiral_token.startswith("dev-") or admiral_token == "dev-token":
