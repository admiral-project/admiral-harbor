...
def sync_catalog(origin="manual", actor=None):
    ...
    try:
        ...
        running = db.session.query(CatalogSyncAudit).filter(
        ...
        missing_apps = CatalogApp.query.filter(
            ...
            CatalogApp.upstream_present == True
        ).all()
...
def get_last_sync():
    ...
    return CatalogSyncAudit.query.filter_by(status="success").order_by(
        CatalogSyncAudit.completed_at.desc()
    ).first()
...
def validate_before_provisioning(app_slug, tier_id):
    ...
    try:
        ...
        return {
            ...
            "message": f"Cannot reach admirald, try again later",
        }
    except Exception as e:
