...
def test_base_url_invalid_mode(app):
    with app.app_context(), patch("app.paypal._db_paypal_config", return_value={"mode": "invalid"}):
...
def test_api_url(app):
    with app.app_context(), patch("app.paypal._db_paypal_config", return_value={"mode": "mock"}):
        url = _api_url()
...
def test_get_access_token_missing_credentials(app):
    with (
        app.app_context(),
        patch("app.paypal._db_paypal_config", return_value={"mode": "sandbox", "client_id": "", "client_secret": ""}),
    ):
        with pytest.raises(PayPalError) as excinfo:
            _get_access_token()
...
def test_get_access_token_request_exception(app):
    with (
        ...
        patch("requests.post", side_effect=requests.RequestException("connection failed")),
    ):
        with pytest.raises(PayPalError) as excinfo:
            _get_access_token()
...
def test_create_subscription_non_mock_success(app):
    with (
        ...
        patch("app.paypal._get_access_token", return_value="token"),
    ):
        mock_response = MagicMock()
        mock_response.json.return_value = {"id": "SUB-123"}
        mock_response.raise_for_status = MagicMock()
        with patch("requests.post", return_value=mock_response) as mock_post:
            res = create_subscription("plan_id", "http://return", "http://cancel", custom_id="cust", amount_cents=1000)
